...
class CameraController(private val context:Context,private val owner:LifecycleOwner,private val view:PreviewView,private val analyzer:AiSceneAnalyzer) {
 private val executor=Executors.newSingleThreadExecutor();private var capture:ImageCapture?=null;private var photoSize:Size?=null
 fun start(error:(String)->Unit){val future=ProcessCameraProvider.getInstance(context);future.addListener({
  try {val provider=future.get();val preview=Preview.Builder().build().also{it.setSurfaceProvider(view.surfaceProvider)}
   capture=ImageCapture.Builder().setCaptureMode(ImageCapture.CAPTURE_MODE_MAXIMIZE_QUALITY).apply{photoSize?.let{setTargetResolution(it)}}.build()
 ...
 /** Rebinds the capture use case to the exact supported JPEG size selected by the user. */
 fun selectPhotoSize(size:Size,error:(String)->Unit){photoSize=size;start(error)}
